...
// sbrCRListGVK is the GroupVersionKind for listing StorageBasedRemediation CRs.
var sbrCRListGVK = schema.GroupVersionKind{
	Group:   sbrparams.CRDGroup,
	Version: sbrparams.CRDVersion,
	Kind:    "StorageBasedRemediationList",
}

// sbrCDetectOnlyTestName is the SBRC name used in the detectOnlyMode test.
const sbrCDetectOnlyTestName = "test-sbrc-detect-only"
	...
	Label(labels.OperatorSBR), func() {
		...
		BeforeAll(func() {
			...
			Eventually(func() error {
				staleCheck := buildSBRC(sbrCDetectOnlyTestName, map[string]interface{}{})

				getErr := APIClient.Get(context.TODO(),
					types.NamespacedName{Name: sbrCDetectOnlyTestName, Namespace: medik8sparams.OperatorNs},
					staleCheck)
				...
				return fmt.Errorf("SBRC %s still terminating", sbrCDetectOnlyTestName)
			...
			existingList.SetGroupVersionKind(sbrCRListGVK)

			existingSBRCRNames = make(map[string]bool)

			Expect(APIClient.List(context.TODO(), existingList,
				&client.ListOptions{Namespace: medik8sparams.OperatorNs})).ToNot(HaveOccurred(),
				"Failed to baseline existing SBR CRs")
			...
			waitForSBRCReady(sbrCDetectOnlyTestName)
		})
		...
		AfterAll(func() {
			...
			deleteErr := APIClient.Delete(context.TODO(), detectOnlySBRC)
			if deleteErr != nil && !k8serrors.IsNotFound(deleteErr) {
				GinkgoT().Logf("Warning: AfterAll cleanup delete %s failed: %v",
					sbrCDetectOnlyTestName, deleteErr)
			}

			By("Waiting for detectOnly StorageBasedRemediationConfig to be fully removed")

			Eventually(func() error {
				getErr := APIClient.Get(context.TODO(),
					types.NamespacedName{Name: sbrCDetectOnlyTestName, Namespace: medik8sparams.OperatorNs},
					buildSBRC(sbrCDetectOnlyTestName, nil))
				if k8serrors.IsNotFound(getErr) {
					return nil
				}
				...
				"SBRC %s must be fully removed during AfterAll cleanup", sbrCDetectOnlyTestName)
		})
		...
		It("Verify SBRC detectOnlyMode field: spec reflection, no remediation CRs, toggle, and DaemonSet GC",
			...
			Label(
				...
				Expect(getErr).ToNot(HaveOccurred(),
					"StorageBasedRemediationConfig %q must exist on the cluster", sbrCDetectOnlyTestName)
				...
				sbrList.SetGroupVersionKind(sbrCRListGVK)
				...
				patchTarget.SetName(sbrCDetectOnlyTestName)
				...
				Expect(deleteErr).ToNot(HaveOccurred(),
					"StorageBasedRemediationConfig %q must be deletable", sbrCDetectOnlyTestName)
