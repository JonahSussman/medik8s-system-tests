	...
	for _, gvk := range []schema.GroupVersionKind{
		...
		for _, object := range list.Items {
			...
			if gvk.Group == "remediation.medik8s.io" || gvk.Group == "self-node-remediation.medik8s.io" ||
				...
				return fmt.Errorf("pre-existing %s %s/%s is not owned by this run", gvk.Kind, object.GetNamespace(), object.GetName())
			}
		}
	}
	return nil
...
func (run *OwnedRun) CreateNamespace(ctx context.Context) error {
	ns := &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: run.Namespace, Labels: map[string]string{
		RunLabel: run.Token,
		"security.openshift.io/scc.podSecurityLabelSync": "false",
		"pod-security.kubernetes.io/enforce":             "privileged",
	}}}
	err := run.API.Create(ctx, ns)
	if err == nil {
		run.NamespaceUID = ns.UID
	}
	return err
}
...
func (run *OwnedRun) Cleanup(ctx context.Context) error {
	...
	ns := &corev1.Namespace{}
	err := run.API.Get(ctx, client.ObjectKey{Name: run.Namespace}, ns)
	...
	if ns.Labels[RunLabel] != run.Token || (run.NamespaceUID != "" && ns.UID != run.NamespaceUID) {
		// No cleanup at all after rejection of a pre-existing namespace.
		if run.NamespaceUID == "" {
			return errors.Join(failures...)
		}
		return errors.Join(append(failures, fmt.Errorf("preserved replaced namespace %s", ns.Name))...)
	}
	...
	if err := DeleteIdentity(ctx, run.API, ns); err != nil {
		failures = append(failures, err)
	}
	// Verify CR and cluster object removal as well as namespace termination.
	remaining := append(clusterObjects, run.Objects...)
	...
	if err != nil {
		failures = append(failures, fmt.Errorf("owned resources remain in/from namespace %s (run %s): %w", run.Namespace, run.Token, err))
	}
	return errors.Join(failures...)
}
...
func SafeSpec(template, namespace, token string) map[string]interface{} {
	return map[string]interface{}{
		...
		"remediationTemplate": map[string]interface{}{"apiVersion": "self-node-remediation.medik8s.io/v1alpha1", "kind": "SelfNodeRemediationTemplate", "name": template, "namespace": namespace},
	}
}
