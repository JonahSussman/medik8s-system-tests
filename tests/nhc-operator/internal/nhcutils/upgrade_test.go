...
func TestRejectedNamespaceNeverRunsCleanup(t *testing.T) {
	ns := &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: "existing", UID: "existing-uid"}}
	api := testClient(ns)
	if err := CheckClean(context.Background(), api, ns.Name); err == nil {
		t.Fatal("accepted existing namespace")
	}
	run := &OwnedRun{API: api, Namespace: ns.Name, Token: "new-run", Packages: []string{"nhc", "snr"},
	...
	if err := api.Get(context.Background(), client.ObjectKeyFromObject(ns), ns); err != nil {
...
func TestCleanupPreservesReplacedNamespace(t *testing.T) {
	ns := &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: "owned", UID: "replacement", Labels: map[string]string{RunLabel: "run"}}}
	api := testClient(ns)
	run := &OwnedRun{API: api, Namespace: ns.Name, NamespaceUID: "original", Token: "run", Packages: []string{"nhc"},
		CleanupPackage: func(context.Context, string, string, string) (string, error) {
...
func TestPartialInstallCleanupContinuesAfterFailure(t *testing.T) {
	ns := &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: "owned", UID: "namespace-uid", Labels: map[string]string{RunLabel: "run"}}}
	api := testClient(ns)
	var attempted []string
	run := &OwnedRun{API: api, Namespace: ns.Name, NamespaceUID: ns.UID, Token: "run", Packages: []string{"snr", "nhc"},
	...
	if err := api.Get(context.Background(), client.ObjectKeyFromObject(ns), ns); !apierrors.IsNotFound(err) {
